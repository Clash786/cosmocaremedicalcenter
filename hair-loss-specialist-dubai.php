...
<head>
	...
	<meta name="description" content="<?php include 'price.php'; echo "$meso"; ?> <?= __t('Hair Loss Treatment in Dubai by Dr. Murtaza Expert Hair & Scalp Specialist at') ?> <?php include 'price.php'; echo "$company"; ?><?= __t('. Contact us at ') ?><?php include 'price.php'; echo "$contact"; ?> <?= __t('(Satwa) / ') ?><?php include 'price.php'; echo "$contactjumeirah"; ?> <?= __t('(Jumeirah 2) or') ?> <?php include 'price.php'; echo "$email"; ?> <?= __t('for appointment.') ?>" />
	<meta name="keywords" content="<?php include 'price.php'; echo "$meso_keywords"; ?>, <?= __t('hair loss treatment dubai, hair regrowth clinic, dermatologist for hair') ?>" />
	<meta name="format-detection" content="telephone=no">
	<?php include('meta.php'); ?>
	<title><?php include 'price.php'; echo "$meso"; ?> <?= __t('Hair Loss Specialist in Dubai |') ?> <?php include 'price.php'; echo "$company"; ?> <?= __t('Satwa Jumeirah 2') ?></title>
	<!-- Stylesheets -->
...
<body class="shop-page">
				...
				<div class="row">
					...
					<div class="col-md-8 col-lg-9 mt-4 mt-md-0">
						<div class="title-wrap">
							...
							<h1>
								<a href="hair-loss-specialist-dubai.php">
									<?php echo $hair_loss_specialist_dubai; ?>
								</a>
							</h1>
							<h3><?= __t('Dubai\'s Leading Hair Loss Expert') ?></h3>
							<h1><?= __t('Regain Your Hair. Restore Confidence. Look Your Best Again.') ?></h1>
							<p><?= __t('At') ?>
								<strong><?= __t('Cosmocare Medical Center') ?></strong><?= __t(', our expert hair loss specialists, led by') ?>
								<strong><?= __t('Dr. Murtaza') ?></strong>, provide advanced treatments for hair
								thinning, hair fall, and other scalp concerns.
								<br><?= __t('Whether you\'re dealing with pattern baldness, stress-related hair loss, or scalp conditions,') ?>
								<strong><?= __t('Dr. Murtaza') ?></strong>
								<?= __t('and the team are here to help you regain healthy, strong hair and restore your confidence.') ?>
							</p>
							<p><?= __t('Our hair loss treatments combine the latest medical techniques with personalized care plans to target the root cause of hair thinning. From PRP therapy and advanced hair regrowth treatments to nutritional guidance, we ensure every patient receives a solution tailored to their needs.') ?>
							</p>
							<p><?= __t('We understand that hair loss can affect confidence and self-esteem. At') ?>
								<strong><?= __t('Cosmocare Medical Center') ?></strong><?= __t(', we are dedicated to providing a comfortable and supportive environment where patients can regain not only their hair but also their confidence and overall well-being.') ?>
							</p>
							<div class="service-img">
								<a href="hair-loss-specialist-dubai.php">
									<img src="images/hair-loss-specialist.jpg"
										alt="Hair Loss Specialist in Dubai by Cosmocare Medical Center"
										style="width:90%; max-width:500px;" border="0" class="img-fluid" />
								</a>
							</div>
							<br>
							<h1><?= __t('Expert Hair Loss Specialist in Dubai') ?></h1>
							<p><?= __t('At') ?>
								<strong><?= __t('Cosmocare Medical Center') ?></strong><?= __t(', we offer advanced hair loss treatments under the expert care of') ?>
								<strong><?= __t('Dr. Murtaza') ?></strong>, one of Dubai's most trusted hair loss
								specialists.
								<br><?= __t('Whether you\'re experiencing hair thinning, hair fall, or scalp issues, Dr. Murtaza and his team are dedicated to restoring your hair\'s health, density, and strength.') ?>
							</p>
							<p><strong><?= __t(' Why Choose Cosmocare for Hair Loss Treatment?') ?></strong></p>
							<ul>
								<li><strong><?= __t(' Personalized Hair Care Plans:') ?></strong>
									<?= __t('Each patient receives a customized treatment plan tailored to their specific hair and scalp condition.') ?>
								</li>
								<li><strong><?= __t(' Advanced Hair Restoration Technology:') ?></strong>
									<?= __t('We use the latest FDA-approved techniques including PRP therapy and hair regrowth solutions for safe and effective results.') ?>
								</li>
								<li><strong><?= __t(' Expert Care by Dr. Murtaza:') ?></strong>
									<?= __t('Over a decade of experience in treating various hair loss problems, including pattern baldness and stress-related hair fall.') ?>
								</li>
								<li><strong><?= __t(' Trusted by Patients Across Dubai:') ?></strong>
									<?= __t('Conveniently located in Satwa and Jumeirah 2, providing affordable and high-quality hair restoration care.') ?>
								</li>
							</ul>
							<p><?= __t(' Book your appointment today with') ?>
								<strong><?= __t('Dr. Murtaza') ?></strong> <?= __t('at') ?>
								<strong><?= __t('Cosmocare Medical Center') ?></strong>
								<?= __t('to regain healthy, strong hair and restore your confidence.') ?></p>
							<p>
	...
	$footer_blogs = [
		...
		[
			...
			'title' => __t('Hair Loss Specialist Dubai...'),
			'link' => 'hair-loss-specialist-dubai.php',
			'date' => 'July 25, 2021'
		],
	];
	$footer_nearby_areas = [
